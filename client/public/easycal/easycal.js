...
;(function($, window, document, _, moment, undefined){
	...
	var Easycal = {
		...
		_renderTimeGridHTML : function(){
			...
			var date = moment(this.options.startDate, format.dateShort);
            var date1 = moment(this.options.startDate, format.dateShort);
			date.isoWeekday(1);
			...
			for(var i = 0 ; i < 8 ; i++){
				if(i===0){
                    ...
                    date1 = date1.format(format.dateShort);
					html += '<td class="ec-slot-col" data-date="' + colDate + '" data-datefor="' + date1 + '">';
					date.add(1, 'd');
                    date1 = moment(date1, "DD-MM-YYYY").add(1, 'days');
				}
